...
class CorelPalette_Methods:
    ...
    def get_palette_name(self):
        if 'name' in list(self.model.attrs.keys()):
            return self.model.attrs['name']
        elif 'resid' in list(self.model.attrs.keys()) \
                and self.model.attrs['resid'] in list(self.localizations.keys()):
    ...
    def convert_color(self, color):
        if color.attrs['cs'] in list(CS_MATCH.keys()):
            cs = CS_MATCH[color.attrs['cs']]
            vals = self.get_color_vals(color.attrs['tints'])
            name = ''
            if 'name' in list(color.attrs.keys()):
                name = color.attrs['name']
            elif 'resid' in list(color.attrs.keys()):
                name = self.localizations[color.attrs['resid']]
            return [cs, vals, 1.0, name]
        elif color.attrs['cs'] in list(self.colorspaces.keys()):
            cs = COLOR_SPOT
