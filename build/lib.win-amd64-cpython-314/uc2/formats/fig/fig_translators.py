...
class SK2_to_FIG_Translator(object):
    ...
    def translate_primitive(self, obj):
        ...
        for path in paths:
            if all([len(a) == 2 for a in path[1]]):
    ...
    def get_fill(self, obj):
        ...
        if not fill:
            ...
            if pattern_type == sk2const.PATTERN_IMG:
                for idx, sample in list(PATTERN.items()):
                    if sample == pattern:
                        area_fill = idx
                        break
            props = dict(
    ...
    def init_pallet(self):
        self.pallet = {}
        for idx, clr in list(FIG_COLORS.items()):
            if idx >= 0:
                hexcolor = cms.rgb_to_hexcolor(clr[1])
    ...
    def translate_pallet(self):
        ...
        pallet = sorted(list(self.pallet.items()), key=lambda a: a[1], reverse=True)
